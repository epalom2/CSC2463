...
let spriteSheetFilename = ["Bug Squish.png"]
let spriteSheets = [];
let animations = [];
...
function preload() {
    for(let i=0; i < spriteSheetFilename.length; i++) {
      spriteSheets[i] = loadImage("libraries/" + spriteSheetFilename[i]);
    }
}
...
function reset() {
    ...
    for(let i=0; i < game.totalSprites; i++) {
      animations[i] = new WalkingAnimation(random(spriteSheets),64,64,random(100,400),random(100,400),8,random(1,1),8,random([0,1]));
    }
}
        ...
        for (let i = 0; i < animations.length; i++) {
          ...
          if (contains) {
            if (animations[i].moving != 0) {
              ...
              // Speed up other bugs
              animations.forEach((otherBug, index) => {
                if (index !== i) otherBug.increaseSpeed(1.15); // Increase speed by 50%
              });
...
class WalkingAnimation {
    constructor(spritesheet, sw, sh, dx, dy, animationLength, speed, framerate, vertical = true, offsetX = 0, offsetY = 0) {
